...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  // Navegação programática (fora de um link da própria gaveta) também fecha.
  useEffect(() => {
    setMenuAberto(false);
  }, [pathname]);
  ...
  return (
    <div className="flex h-dvh bg-bg lg:h-screen">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header
          className="flex shrink-0 items-center gap-3 border-b border-border bg-bg px-4 pb-3 lg:hidden"
